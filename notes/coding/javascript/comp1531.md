...
- Filter = Select from an array
  ...
  const totalMarkSum = passingMarks.reduce((a, b) => a + b, 0);
  const averageMark = totalMarkSum / passingMarks.length;
...
```ts
// Classic
const tutors = ['Simon', 'Teresa', 'Hayden', 'Michelle'];
...
- Takes an array of size N, and produces a new array of size 0..N without modifying any of
  the data.
- Possibly smaller array size
- Elements unchanged
