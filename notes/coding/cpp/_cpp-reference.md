...
- [Containers Library](#containers-library)
  ...
  - [`std::priority_queue`](#stdpriority_queue)
    - [Member Types](#member-types-15)
    - [Member Functions](#member-functions-18)
    - [Member Objects](#member-objects-3)
    - [Non-Member Functions](#non-member-functions-18)
    - [Helper Classes](#helper-classes-9)
    - [Deduction Guide](#deduction-guide-13)
    - [Custom Comparator](#custom-comparator)
      - [Version 1 (`auto cmpFunc`)](#version-1-auto-cmpfunc)
      - [Version 2 (`struct cmpFunc`)](#version-2-struct-cmpfunc)
      - [Version 3 (`class cmpFunc`)](#version-3-class-cmpfunc)
      - [Version 4 (`lambda cmpFunc`)](#version-4-lambda-cmpfunc)
    - [Example](#example-17)
  ...
  - [Lambda Expressions](#lambda-expressions)
    - [Syntax / Parts of a Lambda Expression](#syntax--parts-of-a-lambda-expression)
      - [Capture List](#capture-list)
        ...
        - [Capture Class Member Variable (LeetCode)](#capture-class-member-variable-leetcode)
...
Explanation of why we do NOT need `.begin()` and `.end()` when passing primitive arrays into functions expecting iterators
...
- [LearnCPP](https://www.learncpp.com/cpp-tutorial/stdoptional/)
  - `std::optional` is a class template type that implements an optional value i.e. `std::optional<T>` can either have a value of type `T` or not
...
|             | Observers                      |
|             | `operator->` <br> `operator*`  | Accesses the contained value (public member function)                                                                                 |
|             | `operator bool` `.has_value()` | Checks whether the object contains a value (public member function)                                                                   |
| `T&`        | `.value()`                     | Returns the contained value (public member function)                                                                                  |
...
| `std::optional` | `std::make_optional()`                                                                                                                                                                                                         | Creates an optional object(function template)                         |
| `void`          | `std::swap(std::optional)`                                                                                                                                                                                                     | Specializes the std::swap algorithm (function template)               |
...
// Function that optionally returns an int value
...
int main() {
  ...
  list.emplace_back(std::make_pair("banana", 1.99));
  list.emplace_back(std::make_pair("lychee", 3.99));
  list.emplace_back(std::make_pair("apple", 0.99));
  list.emplace_back(std::make_pair("peach", 2.99));
  for (auto &p : list) {
    std::cout << "The price of " << p.first << " is $" << p.second << '\n';
  }
  // for (auto &[fruit, price] : list) {
  //   std::cout << "The price of " << fruit << " is $" << price << '\n';
  // }
  return 0;
}
```
...
- C++/CPP
  - `.substr(pos, count)` returns a substring with the range `s[pos, pos+count)` (i.e. `s[pos, pos+count-1]`)
  - `.substr(pos)` returns a substring with the range `s[pos, s.size())`
- Java
...
int main() {
  ...
  int i = 6; // size_t i = 6;
  int j = 11; // size_t j = 11;
  // Note: there is NO +1 because we want to count how many indices to go forward from "pos" (hence excluding pos itself)
...
int main() {
  ...
  // std::string sub = s.substr(i, j - i + 1); // std::string sub = s.substr(6, 10-6+1) = s.substr(6, 5)
  // sub == "World"
}
```
...
- See 165.compare-version-numbers.cpp for more examples

```cpp
class Solution {
public:
  int compareVersion(std::string str) {
...
public:
  int compareVersion(std::string version1, std::string version2) {
    ...
    while (!iss1.eof() || !iss2.eof()) {
      if (!iss1.eof()) {
        std::getline(iss1, numStr1, '.');
      } else {
        numStr1 = "0";
      }
      if (!iss2.eof()) {
        std::getline(iss2, numStr2, '.');
      } else {
        numStr2 = "0";
      }
      // Cannot just use the std::string compare func since there could be leading '0's, e.g. '01' vs '1'
...
- We use `.upper_bound()` for BOTH `.floorEntry()` and `.ceilingEntry()` functionalities
  - The reason is because `.lower_bound()` returns an iterator to an element that is GREATER THAN OR EQUAL TO `>=` the given key
    ...
    - This causes a lot of ambiguity on how to find first element `<=` given key
      - Ambiguity: Do we decrement the iterator or not since `it` may point to equivalent key, or first key greater than given key (such as `std::end(map) == map.end()`)
...
- **Note: Comparators are REVERSED for `std::priority_queue` (i.e. `a < b` becomes `a > b`)**
...
// Note: Comparators are REVERSED for std::priority_queue (i.e. `a < b` becomes `a > b`)
auto cmpFunc = [](auto const &a, auto const &b){ return a > b; };
std::priority_queue<T, std::vector<T>, decltype(cmpFunc)> pq(cmpFunc);
```

```cpp
// Max Heap (note: comparators are REVERSED for std::priority_queue)
auto myCmp = [](auto const &a, auto const &b) {
  return a.second < b.second;
};

std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, decltype(myCmp)> pq(myCmp);
```

#### Version 2 (`struct cmpFunc`)
...
struct MyComparator {
  bool operator()(auto const &p1, auto const &p2) const {
    return p1.age < p2.age;
  }
};
...
- The `const` after a function declaration declares that a function is a const member function (will NOT modify any member variables of the class)
- A "const function", denoted with the keyword `const` after a function declaration, makes it a compiler error for this class function to change a data member of the class
  - It permits the reading of class variables inside of the function, but modifying class variables inside of this function will generate a compiler error
...
```cpp
char ch = 's'
std::string str1(1, ch); // s
std::string str2(2, ch); // ss
```
...
std::string str1;
std::string str2;
std::string str1.append(1, ch); // s
std::string str2.append(2, ch); // ss
```
...
std::string str1;
std::string str2;
std::string str1.push_back(ch); // s
```
...
// Note: This method is SLOWER than the methods above
...
- Note: `std::priority_queue<int[]>` is **NOT** supported
  - Fix/Solution: Use `std::priority_queue<std::pair<int, int>>` or `std::priority_queue<std::vector<int>>` instead
- Note: Remember that `Comparators` are REVERSED/FLIPPED for `std::priority_queue`
...
// Note: Comparator is REVERSED/FLIPPED for priority_queue
...
```cpp
//------------------------------------------------------------------------------
// MIN Heap (based on the SECOND element of pair)
//------------------------------------------------------------------------------
// struct CompareFirst {
//   bool operator()(std::pair<int, int> const &a, std::pair<int, int> const &b) const {
//     return a.second > b.second;
...
std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, CompareFirst> minHeapSecond;
```
...
std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, CompareSecond> maxHeapFirst;
```
...
##### Capture Class Member Variable (LeetCode)

- If any errors occur with the capture, place `const` infront (i.e. `[this](const int &num){ ... }`)
...
public:
  ...
  int myFunc(std::vector<int> nums) {
    // Method 1
    std::for_each(nums.begin(), nums.end(), [this](int &num) { this->treeSet.insert(num); });
    // Method 2
    std::for_each(nums.begin(), nums.end(), [&treeSet = this->treeSet](int &num) { treeSet.insert(num); });
    // Method 3
    for (auto it = nums.begin(); it != s.end(); it++) {
      treeSet.insert(*it);
    }
  }
};
...
- Interestingly **passing containers by reference** is **faster** then **creating global containers** and can help avoid TLE
  - **This is because global containers need to be copy constructed**
...
public:
  ...
  bool canDistribute(std::vector<int> &nums, std::vector<int> &requiredQuantity) {
    std::unordered_map<int, int> numToOccurrence;
    for (auto &num : nums) {
      numToOccurrence[num] += 1;
    }
    std::vector<int> counts;
    for (auto &kv : numToOccurrence) {
...
```cpp
std::string replaceWords(vector<std::string> &dictionary, std::string sentence) {
  ...
  while (getline(ss, word, ' ')) {
    newSentence += trie.getShortestRoot(word) + " ";
  }
  // Remove extra space
