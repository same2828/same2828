...
- [COMP6771 Lecture Notes](#comp6771-lecture-notes)
  ...
  - [Resource Management](#resource-management)
    ...
    - [Custom copy assignment operators](#custom-copy-assignment-operators)
    - [Value categories: lvalues and rvalues](#value-categories-lvalues-and-rvalues)
  ...
  - [Smart pointers](#smart-pointers)
    ...
    - [Smart pointers and partial construction](#smart-pointers-and-partial-construction)
  - [Dynamic Polymorphism](#dynamic-polymorphism)
    - [Inheritance](#inheritance)
    - [Object slicing problem](#object-slicing-problem)
    - [Dynamic binding](#dynamic-binding)
    - [Finality](#finality)
    - [Pure virtual functions and abstract classes](#pure-virtual-functions-and-abstract-classes)
    - [OOP type theory](#oop-type-theory)
    - [Polymorphism and construction](#polymorphism-and-construction)
    - [Polymorphism and destruction](#polymorphism-and-destruction)
  - [Templates](#templates)
  ...
  - [Metaprogramming and advanced templates](#metaprogramming-and-advanced-templates)
    ...
    - [Extension: SFINAE](#extension-sfinae)
    - [Template argument deduction](#template-argument-deduction)
    - [Extension: CRTP](#extension-crtp)
  ...
  - [Possible extension topics](#possible-extension-topics)

# COMP6771 Lecture Notes

_These notes sometimes include small technical asides and additions that are not strictly part of COMP6771. They also don't cover all of the content (e.g. testing using `catch2`). You can view these as a concise summary of most of the lecture material that also provide some interesting additional commentary on them._
...
From C++11, there is a new, alternative syntax for functions, called _trailing return type syntax_:
...
Because C++ has value semantics, we have _references_ to give us reference semantics. These are kind of like pointers, but with some differences:
...
(This, however, is _not_ a constant time operation if the iterator is not random-access!)
...
Lambda expresions are, under the hood, actually implemented as anonymous _functors_. A more critical difference between a functor from a function is that functors can have state (e.g. the captured variables in these examples).
...
There are some instances where `using` directives _do_ make sense:
...
By default, members of a class are _private_. The **only** difference between a `struct` and a `class` is that all members of a `struct` are _public_ by default. We will almost always use a `class` unless we essentially just want a data class.
...
Constructors may specify an _initialiser list_, which gives values to data members in order of their member declaration in the class itself:
...
Crucially, this happens _before_ the constructor body is actually executed.
...
- corrupt data
  on exception provides basic exception safety. Objects are afterward left in a _valid but unspecified state_, as there is no telling the extent to which the side effects of partial execution have modified things.
...
These are the two main _value categories_, [but in reality, it is not a strict dichotomy](https://en.cppreference.com/w/cpp/language/value_category).

Loosely speaking, `std::move` can be used to turn an lvalue into an rvalue. (This is a bit of a lie: in reality, what is created is instead an _xvalue_, or expiring value.)
...
_Resource Acquisition Is Initialisation_ is a C++ programming technique in which resources (i.e. heap objects) are encapsulated inside objects, therefore binding the life cycle of an acquired resource to the lifetime of that object. We _acquire_ the resource in the constructor, and we _release_ the resources in the destructor.
...
This is mostly a performance consideration; this default is easily determined at compile time, since derived class object references can be _statically bound_ to base class object references.

However, C++ can be forced to use _dynamic binding_ to determine the right override to pick at runtime using _virtual functions_:
...
Virtual functions rely an under-the-hood abstraction called a _vtable_. Each class has its own vtable, stored in the data segment, consisting of an array of function pointers to the definition of each virtual function.
...
A virtual function is considered to be _pure virtual_ if it has no corresponding implementation in that class:
...
If a class has at least one pure virtual member function, then objects of that class _cannot_ be constructed. Moreover, functions cannot accept or return objects of such a class type by value, only by reference.

Pure virtual functions allow us to mimic the behaviour of _abstract classes_ from other OOP languages.
...
Templated functions/classes _must_ be defined in header files, because template definitions have to be known at compile time. This is in contrast to the usual link-time instantiation we use when writing non-polymorphic code that can separate the interface and implementation freely.
...
A _constant expression_ is a variable that can be calculated at compile time (as `#define`'d values are in C), or a function that, if its inputs are known, can be run at compile time (and its result substituted in place of that function call). We use the `constexpr` keyword to denote such things:
...
- _Partial specialisation_ for a template for a type "based on" a template type parameter (e.g. a specialisation of a template for `T*` or `std::vector<T>`)
