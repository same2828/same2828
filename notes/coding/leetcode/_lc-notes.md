...
- [Exponential](#exponential)
  - [2^x that does NOT Overflow](#2x-that-does-not-overflow)
- [Formulas](#formulas)
...
class Solution {
  ...
  void generateSubsets(int i, List<Integer> currSubset) {
    // Base case: i reached end of nums
    // Add the current currSubset to allSubsets
    if (i == nums.length) {
      allSubsets.add(new ArrayList<>(currSubset));
      return;
    }
    // Generate allSubsets with nums[i]
    currSubset.add(nums[i]);
    generateSubsets(nums, i + 1, currSubset, allSubsets);
    currSubset.remove(currSubset.size() - 1);
    // Generate allSubsets without nums[i]
    generateSubsets(nums, i + 1, currSubset, allSubsets);
  }
}
```
...
| :----------: | :---------: | :---------: | :---------: | :---------: |
|    Power     |    `2^7`    |    `2^6`    |    `2^5`    |    `2^4`    |
|    Value     |    `128`    |    `64`     |    `32`     |    `16`     |
|    Binary    | `1000 0000` | `0100 0000` | `0010 0000` | `0001 0000` |

| Bit Position |        11        |        10        |        9         |        8         |
| :----------: | :--------------: | :--------------: | :--------------: | :--------------: |
|    Power     |      `2^11`      |      `2^10`      |      `2^9`       |      `2^8`       |
...
// Populate binaryRep with 0s and 1s...
int num = 0;
for (int i = n - 1; i >= 0; i--) {
  if (binaryRep[i] == 1) {
...
```java
// V1
reqForZeroOne = 1 - reqForZeroOne;
// V2
reqForZeroOne = remainder == 1 ? 0 : 1;
// V3
reqForZeroOne ^= 1;
```
...
dfsReRoot(currNode = x, parentNode = ..., maxDistUpToCurrNode = ...) {
  for (int[] neiInfo : graph[currNode]) {
    int neiNode = neiInfo[0];
    int weight = neiInfo[1];
    if (neiNode == parentNode) {
      continue;
    }
    // We are calculating maxDistUpToCurrNode for the NEXT dfs()call to neiNode, hence neiNode CANNOT be part of the maximum path distance sum
    int maxDistFromCurrNodeExcludingNeiNode = (maxDist1[currNode] == weight + maxDist1[nei]) ? maxDist2[currNode] : maxDist1[currNode];
    dfsReRoot(neiNode, currNode, weight + std::max(maxDistUpToCurrNode, maxDistFromCurrNodeExcludingNeiNode));
  }
}
```
...
WITHOUT parentheses, this expression would be evaluated as:
...
> **Need to WRAP ternary expression `(booleanExp ? ifTrueExp : ifFalseExp)` in EXTRA set of BRACKETS**
...
int b = 2;
int c = 3;
int result = a + b > c ? b : c; // INVALID
int result = a + (b > c ? b : c); // Valid
```
...
However, we intend to use the ternary operator first so we will NEED to use parentheses:

```java
int result = a + (b > c ? b : c);
```

In this case, the ternary operation `b > c ? b : c` is evaluated FIRST. Since b (10) is not greater than c (15), the result of the ternary operation is c (15). Then, a (5) is added to 15, resulting in 20.

> **Need to ADD modulus to result BEFORE taking modulus**

```java
// INCORRECT/INVALID
// long result = dp(0, 30) % mod;

// Correct/Valid
long result = (dp(0, 30) + mod) % mod;
```
...
int dp(int currCol, int prevVal) {
  ...
  // Use the req-calculated value [negative indexes not allowed in arrays, so skip them]
  if (memo[currCol][prevVal] != -1) {
    return memo[currCol][prevVal];
  }
  int result = Integer.MAX_VALUE;
  for (int currVal = 0; currVal <= 9; currVal++) {
    if (currCol == 0 || currVal != prevVal) { // <- HERE
      // m - cnt[currCol][currVal] == cost to change all values in currCol to currVal, then currVal becomes prevVal for NEXT column
      result = Math.min(result, m - cnt[currCol][currVal] + dp(currCol + 1, currVal));
    }
  }
  return memo[currCol][prevVal] = result;
}
```
...
- Either return 0 or 1 once you reach the end of input array
...
currNum + prevNum = mod + q*k
prevNum = mod + q*k - currNum

Take modulo k of both sides

prevNum % k = (mod + q*k - currNum) % k
prevNum % k = (mod - currNum) % k         (note: q*k % k = 0)
prevNum = (mod - currNum + k) % k         (note: we can remove implicitly remove %k from the LHS, since it is implied/inherent from the RHS that the result will fall under the range[0..k-1])
```
...
If `f(k)` can be written as a sequence, then we can find the equation for `f(k)` by doing:

```tex
% Aim: Solving/Solve for f(k)
f(k) = a + b + ... + y      (1)
f(k-1) = b + c + ... + z    (2)
...
> `aeb == a * 10^b` (where a is the significand/mantissa and b is the exponent)
>
> `1en == 1 * 10^n`

Example: `1e9 + 7 == 1 * 10^9 + 7`
...
class SolutionFast {
  ...
  public int dp(int i, int prev, int k) {
    if (i == nums.length) {
      return memo[i][k][prev + 1] = 0;
    }
    if (memo[i][k][prev + 1] != -1) {
      return memo[i][k][prev + 1];
    }
    int take = 0;
    ...
    int skip = dp(i + 1, prev, k);
    return memo[i][k][prev + 1] = Math.max(take, skip);
  }
}
```
...
class SolutionSlow {
  ...
  public int dp(int i, int prev, int k) {
    if (i == nums.length) {
      return memo[i][prev + 1][k] = 0;
    }
    if (memo[i][prev + 1][k] != -1) {
      return memo[i][prev + 1][k];
    }
    int take = 0;
    if (prev == -1 || nums[i] == nums[prev]) {
      take = 1 + dp(i + 1, i, k);
    ...
    int skip = dp(i + 1, prev, k);
    return memo[i][prev + 1][k] = Math.max(take, skip);
  }
}
```
