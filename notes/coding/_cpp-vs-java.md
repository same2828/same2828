...
- `String` is IMMUTABLE and have special methods (`s.charAt(), s.length()`)
...
- Declaration + Initialisation == `Interface<Type> dataStructure = new ImplementingClass<>();`
  - Note: We NEED to use `new` keyword on RHS
- Does NOT support default argument/parameter values
  - Fix/Solution: Use function OVERLOADING instead
    - `bool trieSearch(string key) {}`
...
- Does support default argument/parameter values
  - `bool trieSearch(string key, bool checkPrefixOnly = false) {}`
...
- `int dp[m][n]` (must provide dimensions upon initialisation)
...
> Use `Integer` type for `null` default initialisation
...
class Solution {
  ...
  public int minFallingPathSum(int[][] matrix) {
    ...
    // Note: We use Integer instead of int because memo[i][j] can be -1 (which breaks default initialisation)
  ...
  public int dp(int i, int j) {
    ...
    // Check if we have reached the last i
    ...
    return memo[i][j] = matrix[i][j] + Math.min(dp(i + 1, j - 1), Math.min(dp(i + 1, j), dp(i + 1, j + 1)));
  }
}
```
...
> Use `std::optional` for `std::nullopt` initialisation
...
public:
  ...
  int minFallingPathSum(std::vector<std::vector<int>> &matrix) {
    int result = INT_MAX;
    // std::vector<std::vector<std::optional<int>>> memo(matrix.size(), std::vector<std::optional<int>>(matrix.size(), std::nullopt));
    this->memo.resize(matrix.size(), std::vector<std::optional<int>>(matrix[0].size(), std::nullopt));
    for (int j = 0; j < matrix.size(); j++) {
      result = std::min(result, dp(0, j, matrix));
    }
    return result;
  }

  int dp(int i, int j, std::vector<std::vector<int>> &matrix) {
    ...
    if (memo[i][j] != std::nullopt) {
      return (memo[i][j]).value_or(0);
    }
    memo[i][j] = matrix[i][j] + std::min(dp(i + 1, j - 1, matrix), std::min(dp(i + 1, j, matrix), dp(i + 1, j + 1, matrix)));
...
- Prefer to use `pair<int, pair<int, int>>` instead of `std::vector` since it is faster (for edge cases where you would use `int[]` in java)
