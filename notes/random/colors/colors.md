# Table of Contents

- [Table of Contents](#table-of-contents)
- [Links](#links)
- [16 ANSI Color Codes](#16-ansi-color-codes)
- [256 ANSI Color Codes](#256-ansi-color-codes)
- [Bash Colors](#bash-colors)
- [Color Palette](#color-palette)
  - [Dracula](#dracula)
  - [Ghostty Dracula](#ghostty-dracula)
- [Color Hex Wiki](#color-hex-wiki)
  - [Apple (macOS)](#apple-macos)
  - [Developer (macOS)](#developer-macos)
  - [Web Safe Colors](#web-safe-colors)
  - [macOS Finder](#macos-finder)
  - [VSCode Dark+](#vscode-dark)
  - [Neon Green](#neon-green)
  - [Orange](#orange)
  - [Random](#random)
  - [Yellow](#yellow)
- [Ghostty](#ghostty)
- [VSCode](#vscode)

# Links

- https://www.figma.com/colors/
- https://encycolorpedia.com/html
- https://materialui.co/htmlcolors
- https://draculatheme.com/contribute
- https://github.com/mbadolato/iTerm2-Color-Schemes/blob/master/ghostty/Dracula

# 16 ANSI Color Codes

| Normal Colors (0-7) | Hex     |
| ------------------- | ------- |
| Black (0)           | #21222C |
| Red (1)             | #FF5555 |
| Green (2)           | #50FA7B |
| Yellow (3)          | #F1FA8C |
| Blue/purple (4)     | #BD93F9 |
| Purple (5)          | #FF79C6 |
| Aqua (6)            | #8BE9FD |
| White (7)           | #F8F8F2 |

| Bright Colors (8-15) | Hex     |
| -------------------- | ------- |
| Bright Black (8)     | #6272A4 |
| Bright Red (9)       | #FF6E6E |
| Bright Green (10)    | #69FF94 |
| Bright Yellow (11)   | #FFFFA5 |
| Bright Blue (12)     | #D6ACFF |
| Bright Purple (13)   | #FF92DF |
| Bright Aqua (14)     | #A4FFFF |
| Bright White (15)    | #FFFFFF |

# 256 ANSI Color Codes

- https://github.com/jonasjacek/colors
- https://www.ditig.com/publications/256-colors-cheat-sheet
- https://upload.wikimedia.org/wikipedia/commons/1/15/Xterm_256color_chart.svg
- https://ss64.com/bash/syntax-colors.html
- https://hexdocs.pm/color_palette/ansi_color_codes.html

# Bash Colors

- https://gist.github.com/vratiu/9780109
- https://gist.github.com/JBlond/2fea43a3049b38287e5e9cefc87b2124

| Color  | String       | String     |
| ------ | ------------ | ---------- |
| Black  | `\033[0;30m` | `\e[0;30m` |
| Red    | `\033[0;31m` | `\e[0;31m` |
| Green  | `\033[0;32m` | `\e[0;32m` |
| Yellow | `\033[0;33m` | `\e[0;33m` |
| Blue   | `\033[0;34m` | `\e[0;34m` |
| Purple | `\033[0;35m` | `\e[0;35m` |
| Cyan   | `\033[0;36m` | `\e[0;36m` |
| White  | `\033[0;37m` | `\e[0;37m` |

| Color       | String       | String     |
| ----------- | ------------ | ---------- |
| Bold Black  | `\033[1;30m` | `\e[1;30m` |
| Bold Red    | `\033[1;31m` | `\e[1;31m` |
| Bold Green  | `\033[1;32m` | `\e[1;32m` |
| Bold Yellow | `\033[1;33m` | `\e[1;33m` |
| Bold Blue   | `\033[1;34m` | `\e[1;34m` |
| Bold Purple | `\033[1;35m` | `\e[1;35m` |
| Bold Cyan   | `\033[1;36m` | `\e[1;36m` |
| Bold White  | `\033[1;37m` | `\e[1;37m` |

# Color Palette

## Dracula

- https://draculatheme.com/contribute

| Color/Token  | Hex     | RGB           | HSL             | Usage                                 |
| ------------ | ------- | ------------- | --------------- | ------------------------------------- |
| Background   | #282A36 | 40, 42, 54    | 231°, 15%, 18%  | Main background                       |
| Comment      | #6272A4 | 98, 114, 164  | 225°, 27%, 51%  | Comments, disabled code               |
| Current Line | #6272A4 | 98, 114, 164  | 225°, 27%, 51%  | Semantic current line highlight token |
| Cyan         | #8BE9FD | 139, 233, 253 | 191°, 97%, 77%  | Classes, types, support, regex        |
| Foreground   | #F8F8F2 | 248, 248, 242 | 60°, 30%, 96%   | Default text                          |
| Green        | #50FA7B | 80, 250, 123  | 135°, 94%, 65%  | Functions, methods, inherited classes |
| Orange       | #FFB86C | 255, 184, 108 | 31°, 100%, 71%  | Numbers, constants, booleans          |
| Pink         | #FF79C6 | 255, 121, 198 | 326°, 100%, 74% | Keywords, storage types               |
| Purple       | #BD93F9 | 189, 147, 249 | 265°, 89%, 78%  | Instance reserved words, constants    |
| Red          | #FF5555 | 255, 85, 85   | 0°, 100%, 67%   | Errors, warnings, deletion            |
| Selection    | #44475A | 68, 71, 90    | 232°, 14%, 31%  | Text selection                        |
| Yellow       | #F1FA8C | 241, 250, 140 | 65°, 92%, 76%   | Strings, text content                 |

## Ghostty Dracula

- https://github.com/mbadolato/iTerm2-Color-Schemes/blob/master/ghostty/Dracula

```
# Normal colors (0-7)
palette = 0=#21222C   # black
palette = 1=#FF5555   # red
palette = 2=#50FA7B   # green
palette = 3=#F1FA8C   # yellow
palette = 4=#BD93F9   # blue/purple #be93f9
palette = 5=#FF79C6   # purple
palette = 6=#8BE9FD   # aqua
palette = 7=#F8F8F2   # white
# Bright colors (8-15)
palette = 8=#6272A4   # bright black
palette = 9=#FF6E6E   # bright red
palette = 10=#69FF94  # bright green
palette = 11=#FFFFA5  # bright yellow
palette = 12=#D6ACFF  # bright blue
palette = 13=#FF92DF  # bright purple
palette = 14=#A4FFFF  # bright aqua
palette = 15=#FFFFFF  # bright white
background = #282A36
foreground = #F8F8F2
cursor-color = #F8F8F2
cursor-text = #282A36
selection-background = #44475A
selection-foreground = #FFFFFF
```

# Color Hex Wiki

## Apple (macOS)

| Color   | Hex     |
| ------- | ------- |
| Blue    | #0433FF |
| Cyan    | #00FDFF |
| Green   | #00F900 |
| Magenta | #FF40FF |
| Orange  | #FF9300 |
| Purple  | #942192 |
| Red     | #FF2600 |
| Yellow  | #FFFB00 |

## Developer (macOS)

| Color                       | Hex       |
| --------------------------- | --------- |
| Control Accent              | #007AFF   |
| Control Text                | #000000D8 |
| Disabled Control Text       | #0000003F |
| Find                        | #FFFF00   |
| Keyboard Focus Indicator    | #0067F47F |
| Link                        | #0068DA   |
| Selected Content Background | #0064E1   |
| Selected Control            | #B3D7FF   |
| Selected Control Text       | #000000D8 |
| Selected Text Background    | #B3D7FF   |
| System Blue                 | #007AFF   |
| System Brown                | #A2845E   |
| System Cyan                 | #55BEF0   |
| System Gray                 | #8E8E93   |
| System Green                | #28CD41   |
| System Indigo               | #5856D6   |
| System Mint                 | #00C7BE   |
| System Orange               | #FF9500   |
| System Pink                 | #FF2D55   |
| System Purple               | #AF52DE   |
| System Red                  | #FF3B30   |
| System Teal                 | #59ADC4   |
| System Yellow               | #FFCC00   |

## Web Safe Colors

```
"web-safe-colors": [
  "#FFFFFF",
  "#CCCCCC",
  "#999999",
  "#666666",
  "#333333",
  "#000000",
  "#FFCCCC",
  "#CC9999",
  "#996666",
  "#663333",
  "#330000",
  "#FF9999",
  "#CC6666",
  "#CC3333",
  "#993333",
  "#660000",
  "#FF6666",
  "#FF3333",
  "#FF0000",
  "#CC0000",
  "#990000",
  "#FF9966",
  "#FF6633",
  "#FF3300",
  "#CC3300",
  "#993300",
  "#FFCC99",
  "#CC9966",
  "#CC6633",
  "#996633",
  "#663300",
  "#FF9933",
  "#FF6600",
  "#FF9900",
  "#CC6600",
  "#CC9933",
  "#FFCC66",
  "#FFCC33",
  "#FFCC00",
  "#CC9900",
  "#996600",
  "#FFFFCC",
  "#CCCC99",
  "#999966",
  "#666633",
  "#333300",
  "#FFFF99",
  "#CCCC66",
  "#CCCC33",
  "#999933",
  "#666600",
  "#FFFF66",
  "#FFFF33",
  "#FFFF00",
  "#CCCC00",
  "#999900",
  "#CCFF66",
  "#CCFF33",
  "#CCFF00",
  "#99CC00",
  "#669900",
  "#CCFF99",
  "#99CC66",
  "#99CC33",
  "#669933",
  "#336600",
  "#99FF33",
  "#99FF00",
  "#66FF00",
  "#66CC00",
  "#66CC33",
  "#99FF66",
  "#66FF33",
  "#33FF00",
  "#33CC00",
  "#339900",
  "#CCFFCC",
  "#99CC99",
  "#669966",
  "#336633",
  "#003300",
  "#99FF99",
  "#66CC66",
  "#33CC33",
  "#339933",
  "#006600",
  "#66FF66",
  "#33FF33",
  "#00FF00",
  "#00CC00",
  "#009900",
  "#66FF99",
  "#33FF66",
  "#00FF33",
  "#00CC33",
  "#009933",
  "#99FFCC",
  "#66CC99",
  "#33CC66",
  "#339966",
  "#006633",
  "#33FF99",
  "#00FF66",
  "#00FF99",
  "#00CC66",
  "#33CC99",
  "#66FFCC",
  "#33FFCC",
  "#00FFCC",
  "#00CC99",
  "#009966",
  "#CCFFFF",
  "#99CCCC",
  "#669999",
  "#336666",
  "#003333",
  "#99FFFF",
  "#66CCCC",
  "#33CCCC",
  "#339999",
  "#006666",
  "#66FFFF",
  "#33FFFF",
  "#00FFFF",
  "#00CCCC",
  "#009999",
  "#66CCFF",
  "#33CCFF",
  "#00CCFF",
  "#0099CC",
  "#006699",
  "#99CCFF",
  "#6699CC",
  "#3399CC",
  "#336699",
  "#003366",
  "#3399FF",
  "#0099FF",
  "#0066FF",
  "#0066CC",
  "#3366CC",
  "#6699FF",
  "#3366FF",
  "#0033FF",
  "#0033CC",
  "#003399",
  "#CCCCFF",
  "#9999CC",
  "#666699",
  "#333366",
  "#000033",
  "#9999FF",
  "#6666CC",
  "#3333CC",
  "#333399",
  "#000066",
  "#6666FF",
  "#3333FF",
  "#0000FF",
  "#0000CC",
  "#000099",
  "#9966FF",
  "#6633FF",
  "#3300FF",
  "#3300CC",
  "#330099",
  "#CC99FF",
  "#9966CC",
  "#6633CC",
  "#663399",
  "#330066",
  "#9933FF",
  "#6600FF",
  "#9900FF",
  "#6600CC",
  "#9933CC",
  "#CC66FF",
  "#CC33FF",
  "#CC00FF",
  "#9900CC",
  "#660099",
  "#FFCCFF",
  "#CC99CC",
  "#996699",
  "#663366",
  "#330033",
  "#FF99FF",
  "#CC66CC",
  "#CC33CC",
  "#993399",
  "#660066",
  "#FF66FF",
  "#FF33FF",
  "#FF00FF",
  "#CC00CC",
  "#990099",
  "#FF66CC",
  "#FF33CC",
  "#FF00CC",
  "#CC0099",
  "#990066",
  "#FF99CC",
  "#CC6699",
  "#CC3399",
  "#993366",
  "#660033",
  "#FF3399",
  "#FF0099",
  "#FF0066",
  "#CC0066",
  "#CC3366",
  "#FF6699",
  "#FF3366",
  "#FF0033",
  "#CC0033",
  "#990033"
]
```

## macOS Finder

- #0c58d1
- #3E638C
- #016add (Alfred)
- #004daa

## VSCode Dark+

- #4DC9B0

## Neon Green

- #2CFF05
- #00FF00

## Orange

- Yellow Orange: #FFAA33

## Random

- #00FFCC
- #00FFFF

## Yellow

- Figma Gold: #EFBF04
- Mikado Yellow: #FDBE0F

# Ghostty

```
palette = 0=#000000
palette = 1=#FF0000
palette = 10=#00F900
palette = 11=#FFFB00
# palette = 12=#BD93F9
# palette = 12=#00AFF0
# palette = 12=#00CCFF
# palette = 12=#00BFFF
# palette = 12=#17A9FD
palette = 12=#B3D7FF
palette = 13=#FF40FF
palette = 14=#00FFFF
palette = 15=#FFFFFF
palette = 2=#00F900
palette = 3=#FFFB00
# palette = 4=#BD93F9
# palette = 4=#00AFF0
# palette = 4=#00CCFF
# palette = 4=#00BFFF
# palette = 4=#17A9FD
palette = 4=#B3D7FF
palette = 5=#FF40FF
palette = 6=#00FFFF
palette = 7=#FFFFFF
palette = 8=#000000
palette = 9=#FF0000
```

# VSCode

```
"list.highlightForeground": "#00BBFF",
"list.highlightForeground": "#00AFF0",
"list.highlightForeground": "#1DA1F2",
"list.highlightForeground": "#009EDB",
"list.highlightForeground": "#0099FF",
"list.highlightForeground": "#2AAAFF",
"list.highlightForeground": "#EFBF04",
"list.highlightForeground": "#50FA7B",
```

```
"list.activeSelectionBackground": "#054A78",

#0060C0
#004a94

#0060C0
#005EBC
#005CB8
#005AB4
#0058B0
#0056AC
#0054A8
#0052A4
#0050A0
#004E9C
#004C98
#004A94

#0050A0
#004F9E
#004E9C
```
